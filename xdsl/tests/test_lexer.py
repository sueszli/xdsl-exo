...
def assert_single_token(input: str, expected_kind: MLIRTokenKind, expected_text: str | None = None):
...
@pytest.mark.parametrize("text", ['""', '"@"', '"foo"', '"\\""', '"\\n"', '"\\\\"', '"\\t"'])
...
@pytest.mark.parametrize("text", ["a", "A", "_", "a_", "a1", "a1_", "a1_2", "a1_2_3", "a$_.", "a$_.1"])
...
@pytest.mark.parametrize("text", ["0", "1234", "a", "S", "$", "_", ".", "-", "e_.$-324", "e5$-e_", "foo"])
...
@pytest.mark.parametrize("text,expected", [("0a", "0"), ("0xg", "0"), ("0xfg", "0xf"), ("0xf.", "0xf")])
def test_integer_literal_split(text: str, expected: str):
    assert_single_token(text, MLIRTokenKind.INTEGER_LIT, expected)


@pytest.mark.parametrize("text", ["0.", "1.", "0.2", "38.1243", "92.54e43", "92.5E43", "43.3e-54", "32.E+25"])
def test_float_literal(text: str):
    assert_single_token(text, MLIRTokenKind.FLOAT_LIT)


@pytest.mark.parametrize("text,expected", [("3.9e", "3.9"), ("4.5e+", "4.5"), ("5.8e-", "5.8")])
