...
from xdsl.dialects.hw import HW, Direction, DirectionAttr, HWModuleLike, HWModuleOp, InnerRefAttr, InnerRefNamespaceTrait, InnerRefUserOpInterfaceTrait, InnerSymAttr, InnerSymbolTableCollection, InnerSymbolTableTrait, InnerSymPropertiesAttr, InnerSymTarget, InstanceOp, ModulePort, ModuleType
from xdsl.dialects.test import TestOp
from xdsl.ir import Block
from xdsl.irdl import IRDLOperation, Region, attr_def, irdl_op_definition, opt_region_def, region_def, traits_def
from xdsl.parser import Parser
from xdsl.traits import IsTerminator, SingleBlockImplicitTerminator, SymbolOpInterface, SymbolTable, ensure_terminator
...
def test_inner_symbol_table_interface():
    ...
    """
    mod = ModuleOp(attributes={"sym_name": StringAttr("symbol_name")}, regions=[[OutputOp()]])

    circ = CircuitOp(attributes={"sym_name": StringAttr("other_name")}, regions=[[mod]])
    circ.verify()

    mod_no_parent = ModuleOp(attributes={"sym_name": StringAttr("symbol_name")}, regions=[[OutputOp()]])
    with pytest.raises(
        VerifyException,
    ...
    mod_no_trait_circ = ModuleOp(attributes={"sym_name": StringAttr("symbol_name")}, regions=[[OutputOp()]])
    ...
    mod_missing_trait = MissingTraitModuleOp(attributes={"sym_name": StringAttr("symbol_name")}, regions=[[OutputOp()]])
    circ_mod_missing_trait = CircuitOp(regions=[[mod_missing_trait, OutputOp()]])
    with pytest.raises(VerifyException, match="Operation module must have trait SymbolOpInterface"):
...
def test_inner_ref_namespace_interface():
    ...
    class MissingTraitCircuitOp(IRDLOperation):
        ...
        region: Region | None = opt_region_def()
        sym_name = attr_def(StringAttr)
        traits = traits_def(InnerRefNamespaceTrait(), SingleBlockImplicitTerminator(OutputOp))

    wire0 = WireOp(attributes={"sym_name": StringAttr("wire0")})
    mod0 = MissingTraitCircuitOp(attributes={"sym_name": StringAttr("mod0")}, regions=[[wire0, OutputOp()]])
    circuit_no_symboltable = CircuitOp(attributes={"sym_name": StringAttr("circuit")}, regions=[[mod0]])

    with pytest.raises(VerifyException, match="Operation circuit must have trait SymbolTable"):
    ...
    mod1 = ModuleOp(attributes={"sym_name": StringAttr("mod1")}, regions=[[wire1, OutputOp()]])
    mod2 = ModuleOp(attributes={"sym_name": StringAttr("mod2")}, regions=[[wire2, OutputOp()]])
    circuit = CircuitOp(attributes={"sym_name": StringAttr("circuit")}, regions=[[mod1, mod2]])

    # InnerRefUserOpInterfaceTrait.verify_inner_refs() does not do anything, so just mock
    # to check it is called
    with patch.object(InnerRefUserOpInterfaceTrait, "verify_inner_refs") as inner_ref_verif:
        circuit.verify()
...
def test_inner_symbol_table_collection():
    ...
    with pytest.raises(VerifyException, match="Operation wire should have InnerRefNamespaceTrait trait"):
        inner_sym_tables = InnerSymbolTableCollection(wire1)

    inner_sym_tables = InnerSymbolTableCollection(circuit)

    with pytest.raises(VerifyException, match=r"Trying to insert the same op twice in symbol tables:"):
        inner_sym_tables.populate_and_verify_tables(circuit)

    with pytest.raises(VerifyException, match="Operation wire should have InnerSymbolTableTrait trait"):
    ...
    assert sym_table1 is not sym_table2, "Different InnerSymbolTableTrait objects must return different instances of inner symbol tables"
    ...
    assert sym_table3 is not sym_table4, "InnerSymbolTableTrait still behave as expected when created on the fly"
...
def test_inner_ref_attr():
    ...
    assert ref.get_module().data == "mod2", "Name of the referenced module should be returned correctly"
...
def test_inner_sym_attr():
    ...
    assert invalid_sym_attr.get_sym_name() is None, "Invalid InnerSymAttr should return no name"

    sym_attr = InnerSymAttr("sym")
    assert sym_attr.get_sym_name() == StringAttr("sym"), "InnerSymAttr for “ground” type should return name"
    ...
    assert aggregate_sym_attr.get_sym_name() == StringAttr("sym"), "InnerSymAttr for aggregate types should return name with field ID 0"

    for inner, expected_field_id in zip(aggregate_sym_attr, [0, 1, 2]):
        assert inner.field_id.data == expected_field_id, "InnerSymAttr should allow iterating its properties in order"

    aggregate_without_nested = aggregate_sym_attr.erase(2)
    assert aggregate_without_nested.get_sym_if_exists(2) is None, "InnerSymAttr removal should work"
    assert len(aggregate_without_nested) == 2, "InnerSymAttr removal should correctly change length"
...
def test_hwmoduleop_hwmodulelike():
    ...
    hw_module = HWModuleOp(StringAttr("foo"), module_type, Region((Block((OutputOp(),)),)))
    ...
    new_module_type = ModuleType((ArrayAttr((ModulePort((StringAttr("in1"), i32, DirectionAttr(Direction.INPUT))),)),))
