...
dest_dsd = GetMemDsdOp(operands=[tensor, size_i32], result_types=[DsdType(DsdKind.mem1d_dsd)])
src_dsd1 = GetMemDsdOp(operands=[tensor, size_i32], result_types=[DsdType(DsdKind.mem1d_dsd)])
src_dsd2 = GetMemDsdOp(operands=[tensor, size_i32], result_types=[DsdType(DsdKind.mem1d_dsd)])
...
def test_verify_invalid_builtin_signature():
    with pytest.raises(VerifyException):
        Add16Op(operands=[(dest_dsd, src_dsd1, src_dsd2, dest_dsd)], result_types=[]).verify_()
    with pytest.raises(VerifyException):
