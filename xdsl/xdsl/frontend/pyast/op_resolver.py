...
class OpResolver:
    ...
    def resolve_op(module_name: str, func_name: str) -> Callable[..., Operation]:
        ...
        if not hasattr(module, resolver_name):
            raise FrontendProgramException(f"Internal failure: operation '{func_name}' does not exist " f"in module '{module_name}'.")
        return getattr(module, resolver_name)()

    @staticmethod
    def resolve_op_overload(python_op: str, frontend_type: type[_FrontendType]) -> Callable[..., Operation]:
        # First, get overloaded function.
        if not hasattr(frontend_type, python_op):
            raise FrontendProgramException(f"Internal failure: '{frontend_type.__name__}' does not " f"overload '{python_op}'.")
        ...
        if len(func_ast.body) != 2 or not isinstance(func_ast.body[0], ast.ImportFrom) or not isinstance(func_ast.body[1], ast.Return) or not isinstance(func_ast.body[1].value, ast.Call) or not isinstance(func_ast.body[1].value.func, ast.Name):
            msg = f"""
