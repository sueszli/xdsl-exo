...
class LinalgFunctions(InterpreterFunctions):
    ...
    def run_generic(self, interpreter: Interpreter, op: linalg.GenericOp, args: tuple[Any, ...]) -> PythonValues:
        if op.library_call is not None:
            raise NotImplementedError("library_call not yet supported in linalg.generic interpreter")
        if op.res:
            raise NotImplementedError("results not yet supported in linalg.generic interpreter")
        ...
        for indices in product(*(range(loop_range) for loop_range in loop_ranges)):
            loop_args = tuple(((cast(ShapedArray[Any], i)).load(indexing_map.eval(indices, ())) if isinstance(i, ShapedArray) else i) for i, indexing_map in zip(args, indexing_maps, strict=True))
    ...
    def run_yield(self, interpreter: Interpreter, op: linalg.YieldOp, args: tuple[Any, ...]):
        return ReturnedValues(args), ()

    @impl(linalg.AddOp)
    def run_add(self, interpreter: Interpreter, op: linalg.AddOp, args: tuple[Any, ...]) -> tuple[Any, ...]:
    ...
    @impl(linalg.FillOp)
    def run_fill(self, interpreter: Interpreter, op: linalg.FillOp, args: tuple[Any, ...]) -> tuple[Any, ...]:
    ...
    @impl(linalg.MulOp)
    def run_mul(self, interpreter: Interpreter, op: linalg.MulOp, args: tuple[Any, ...]) -> tuple[Any, ...]:
        lhs, rhs, res = args[0], args[1], args[2]
        assert isinstance(lhs, ShapedArray)
        assert isinstance(rhs, ShapedArray)
        assert isinstance(res, ShapedArray)
    ...
    def run_transpose(self, interpreter: Interpreter, op: linalg.TransposeOp, args: tuple[Any, ...]) -> tuple[Any, ...]:
        operand, res = args[0], args[1]
        assert isinstance(operand, ShapedArray)
        assert isinstance(res, ShapedArray)
        operand = cast(ShapedArray[float], operand)
    ...
    def run_mat_mul(self, interpreter: Interpreter, op: linalg.MatmulOp, args: tuple[Any, ...]) -> tuple[Any, ...]:
        lhs, rhs, res = args[0], args[1], args[2]
        assert isinstance(lhs, ShapedArray)
        assert isinstance(rhs, ShapedArray)
        assert isinstance(res, ShapedArray)
        ...
        for i in range(rows):
            for j in range(cols):
                res.data_ptr[i * cols + j] = sum(lhs.data_ptr[i * lhs.shape[1] + k] * rhs.data_ptr[k * rhs.shape[1] + j] for k in range(lhs.shape[1]))
    ...
    def run_pooling_nchw_max(
        ...
        input_data = [[input.data[r * m_width + c] for c in range(m_width)] for r in range(m_height)]
    ...
    def run_conv_2d_nchw_fchw(
        ...
        kernel_data = [[kernel_filter.data[r * kernel_filter.shape[3] + c] for c in range(kernel_filter.shape[3])] for r in range(kernel_filter.shape[2])]
        output: list[float] = []
        for k in range(0, m_height - ky + 1, strides[0]):
            for l in range(0, m_width - kx + 1, strides[0]):
