...
class MemRefStreamFunctions(InterpreterFunctions):
    ...
    def run_generic(
        ...
        if memref_stream.IteratorTypeAttr.interleaved() in op.iterator_types:
            raise NotImplementedError("Interpreter for interleaved operations not yet implemented")

        inputs_count = len(op.inputs)
        outputs_count = len(op.outputs)

        outputs: tuple[ShapedArray[int | float], ...] = args[inputs_count : inputs_count + outputs_count]
        ...
        if inner_ubs:
            ...
            for outer_indices in product(*(range(outer_ub) for outer_ub in outer_ubs)):
                output_loop_args = tuple(
                    (o.load(indexing_map.eval(outer_indices, ())) if init is None else init)
                ...
                for inner_indices in product(*(range(inner_ub) for inner_ub in inner_ubs)):
                    input_loop_args = tuple(((cast(ShapedArray[Any], i)).load(indexing_map.eval(outer_indices + inner_indices, ())) if isinstance(i, ShapedArray) else i) for i, indexing_map in zip(inputs, input_indexing_maps, strict=True))

                    loop_results = interpreter.run_ssacfg_region(op.body, input_loop_args + output_loop_args, "for_loop")
                    output_loop_args = loop_results
                for res, indexing_map, output in zip(output_loop_args, output_indexing_maps, outputs, strict=True):
            ...
            for indices in product(*(range(outer_ub) for outer_ub in outer_ubs)):
                ...
                for res, indexing_map, output in zip(loop_results, output_indexing_maps, outputs, strict=True):
    ...
    def run_yield(self, interpreter: Interpreter, op: memref_stream.YieldOp, args: tuple[Any, ...]):
