...
class RiscvFuncFunctions(InterpreterFunctions):
    ...
    def run_return(self, interpreter: Interpreter, op: riscv_func.ReturnOp, args: tuple[Any, ...]) -> tuple[TerminatorValue, PythonValues]:
        args = RiscvFunctions.get_reg_values(interpreter, op.operands, args)
        return ReturnedValues(args), ()

    @impl(riscv_func.CallOp)
    def run_call(self, interpreter: Interpreter, op: riscv_func.CallOp, args: tuple[Any, ...]) -> tuple[Any, ...]:
        args = RiscvFunctions.get_reg_values(interpreter, op.operands, args)
    ...
    def run_func(self, interpreter: Interpreter, op: riscv_func.FuncOp, args: tuple[Any, ...]) -> tuple[Any, ...]:
        if (first_block := op.body.blocks.first) is None or not first_block.ops:
            ...
            args = RiscvFunctions.set_reg_values_for_types(interpreter, op.function_type.inputs.data, args)
