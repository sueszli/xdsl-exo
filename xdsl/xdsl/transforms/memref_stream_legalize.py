...
from xdsl.dialects.builtin import AffineMapAttr, ArrayAttr, Float16Type, Float32Type, Float64Type, IntegerAttr, ModuleOp, VectorType, f16, f32
...
StreamingVectorLegalizationType: TypeAlias = VectorType[Float64Type | Float32Type | Float16Type]
...
def _is_safe_to_legalize(op: Operation) -> bool:
    ...
    return isinstance(
        op,
        memref_stream.YieldOp | arith.MinimumfOp | arith.MinnumfOp | arith.MaximumfOp | arith.MaxnumfOp | arith.AddfOp | arith.SubfOp | arith.MulfOp | arith.DivfOp | arith.NegfOp,
    )


def _legalize_block(block: Block, to_be_legalized: set[Operation], rewriter: PatternRewriter) -> None:
    ...
    for op in block.ops:
        ...
        if not _is_safe_to_legalize(op):
            continue
        illegal_results: list[int] = [result.index for result in op.results if not _is_legal_attr(result.type)]
        ...
        to_be_legalized.update(use.operation for idx in illegal_results for use in new_op.results[idx].uses)
...
class MemRefStreamGenericLegalize(RewritePattern):
    ...
    def match_and_rewrite(self, op: memref_stream.GenericOp, rewriter: PatternRewriter) -> None:
        ...
        if op.iterator_types.data[-1].data != IteratorType.PARALLEL:
            raise DiagnosticException("iterators other than 'parallel' are not supported yet")
        ...
        for i, v in legalizations.items():
            ...
            if innermost_bound % n_lanes != 0:
                raise ValueError(f"no. of vector lanes ({n_lanes}) introduced to legalize argument #{i} " f"is not a divisor for the innermost dimension's bound ({innermost_bound})")
            vector_lengths.add(n_lanes)
        if len(vector_lengths) != 1:
            # FIXME we should deal with heterogeneous generic ops
            raise NotImplementedError("cannot legalize heterogeneous block arguments yet")
        ...
        new_maps = tuple(AffineMapAttr(m.data.replace_dims_and_symbols(new_dims, (), len(new_dims), 0)) for m in op.indexing_maps)
