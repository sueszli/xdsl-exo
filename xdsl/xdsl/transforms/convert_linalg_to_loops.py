...
from xdsl.transforms.loop_nest_lowering_utils import indices_for_map, rewrite_generic_to_loops
...
class LowerGenericOpPattern(RewritePattern):
    ...
    def match_and_rewrite(self, op: linalg.GenericOp, rewriter: PatternRewriter) -> None:
        if op.res:
            raise NotImplementedError("lowering for linalg.generic with results not yet supported")
        ...
        def insert_load(
            ...
            if isinstance(value.type, MemRefType):
                indices = indices_for_map(rewriter, insertion_target, affine_map_attr.data, ind_vars)
