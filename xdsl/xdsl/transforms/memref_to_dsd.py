...
from xdsl.dialects.builtin import AffineMapAttr, ArrayAttr, Float16Type, Float32Type, IntAttr, IntegerAttr, MemRefType, ModuleOp, NoneAttr, StridedLayoutAttr, UnrealizedConversionCastOp, i8, i16
...
class LowerAllocOpPass(RewritePattern):
    ...
    def match_and_rewrite(self, op: memref.AllocOp, rewriter: PatternRewriter, /):
        assert MemRefType[ZerosOpAttr].constr(element_type=csl.ZerosOpAttrConstr).verifies(memref_type := op.memref.type)
        zeros_op = csl.ZerosOp(memref_type)

        dsd_t = csl.DsdType(csl.DsdKind.mem1d_dsd if len(memref_type.shape) == 1 else csl.DsdKind.mem4d_dsd)
        offsets = None
        if isinstance(memref_type.layout, StridedLayoutAttr) and isinstance(memref_type.layout.offset, IntAttr):
...
class FixGetDsdOnGetDsd(RewritePattern):
    ...
    def match_and_rewrite(self, op: csl.GetMemDsdOp, rewriter: PatternRewriter, /):
        if isinstance(op.base_addr.type, csl.DsdType):
            if isinstance(op.base_addr, OpResult) and isinstance(op.base_addr.op, csl.GetMemDsdOp):
...
class FixMemRefLoadOnGetDsd(RewritePattern):
    ...
    def match_and_rewrite(self, op: memref.LoadOp, rewriter: PatternRewriter, /):
        if isinstance(op.memref.type, csl.DsdType):
            if isinstance(op.memref, OpResult) and isinstance(op.memref.op, csl.GetMemDsdOp):
                rewriter.replace_matched_op(memref.LoadOp.get(op.memref.op.base_addr, op.indices))
            else:
...
class LowerSubviewOpPass(RewritePattern):
    ...
    def match_and_rewrite(self, op: memref.SubviewOp, rewriter: PatternRewriter, /):
        ...
        if len(op.result.type.get_shape()) == 1 and len(op.source.type.get_shape()) > 1:
            ...
            assert len(counter_sizes) == 1, "1d access into nd memref must specify one size > 1"
            size, size_count = counter_sizes.most_common()[0]
            size = cast(int, size)

            assert size_count == 1, "1d access into nd memref can only specify one size > 1, which can occur only once"
            assert all(stride == 1 for stride in op.static_strides.get_values()), "All strides must equal 1"

            amap: list[AffineExpr] = [AffineConstantExpr(cast(int, o) if o != memref.SubviewOp.DYNAMIC_INDEX else 0) for o in op.static_offsets.get_values()]
            ...
            dsd_op = csl.GetMemDsdOp(
                ...
                properties={"tensor_access": AffineMapAttr(AffineMap(1, 0, tuple(amap)))},
    ...
    @staticmethod
    def _update_sizes(subview: memref.SubviewOp, curr_op: SSAValue | Operation) -> list[Operation]:
        ...
        if static_sizes[0] == memref.SubviewOp.DYNAMIC_INDEX:
            ...
            ops.append(curr_op := csl.SetDsdLengthOp.build(operands=[curr_op, cast_op], result_types=[subview.source.type]))
            ...
            ops.append(curr_op := csl.SetDsdLengthOp.build(operands=[curr_op, len_op], result_types=[subview.source.type]))
        return ops

    @staticmethod
    def _update_strides(subview: memref.SubviewOp, curr_op: SSAValue | Operation) -> list[Operation]:
        assert isa(subview.source.type, MemRefType[Attribute])
        ops = list[Operation]()
        ...
        if static_strides[0] == memref.SubviewOp.DYNAMIC_INDEX:
            ...
            ops.append(csl.SetDsdStrideOp.build(operands=[curr_op, cast_op], result_types=[subview.source.type]))
            ...
            ops.append(csl.SetDsdStrideOp.build(operands=[curr_op, stride_op], result_types=[subview.source.type]))
        return ops

    @staticmethod
    def _update_offsets(subview: memref.SubviewOp, curr_op: SSAValue | Operation) -> list[Operation]:
        assert isa(subview.source.type, MemRefType[Attribute])
        ops = list[Operation]()
        ...
        elif isinstance(subview.source.type.layout, StridedLayoutAttr) and static_offsets[0] != (subview.source.type.layout.get_offset() or 0) or isinstance(subview.source.type.layout, NoneAttr) and static_offsets[0] != 0:
...
class LowerUnrealizedConversionCastOpPass(RewritePattern):
    ...
    def match_and_rewrite(self, op: UnrealizedConversionCastOp, rewriter: PatternRewriter, /):
        if all(isa(t, csl.DsdType) for t in op.inputs.types) and all(isa(t, MemRefType[Attribute]) for t in op.outputs.types):
...
class RetainAddressOfOpPass(RewritePattern):
    ...
    def match_and_rewrite(self, op: csl.AddressOfOp, rewriter: PatternRewriter, /):
        if isinstance(op.value.type, csl.DsdType) and isinstance(op.value.owner, csl.GetMemDsdOp):
            rewriter.replace_matched_op(csl.AddressOfOp.build(operands=[op.value.owner.base_addr], result_types=op.result_types))
...
class CslVarUpdate(RewritePattern):
    ...
    def match_and_rewrite(self, op: csl.VariableOp, rewriter: PatternRewriter, /):
        if not isa(elem_t := op.res.type.get_element_type(), MemRefType[Attribute]) or op.default:
            return
        dsd_t = csl.DsdType(csl.DsdKind.mem1d_dsd if len(elem_t.shape) == 1 else csl.DsdKind.mem4d_dsd)
...
class CslVarLoad(RewritePattern):
    ...
    def match_and_rewrite(self, op: csl.LoadVarOp, rewriter: PatternRewriter, /):
        if not isa(op.res.type, MemRefType[Attribute]) or not isinstance(op.var.type, csl.VarType) or not isa(op.var.type.get_element_type(), csl.DsdType):
            return
