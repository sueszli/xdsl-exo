...
class ScfParallelLoopTilingPattern(RewritePattern):
    ...
    def match_and_rewrite(self, op: ParallelOp, rewriter: PatternRewriter, /):
        ...
        tile_sizes_v = self.tile_sizes[: len(lower)] + (1,) * (len(lower) - len(self.tile_sizes))

        zero = arith.ConstantOp(IntegerAttr.from_index_int_value(0))
        tile_sizes_v = {i: s for i, s in enumerate(tile_sizes_v) if s != 0}
        tile_sizes = {i: arith.ConstantOp(IntegerAttr.from_index_int_value(s)) for i, s in tile_sizes_v.items()}
        ...
        for i in range(len(op.lowerBound)):
            if i in tile_sizes:
                ...
                if isinstance(ilower, arith.ConstantOp) and isinstance(iupper, arith.ConstantOp) and isinstance(istep, arith.ConstantOp):
                    lower_v, upper_v, step_v = (c.value for c in (ilower, iupper, istep))
                    assert isa(lower_v, IntegerAttr[IndexType])
                    assert isa(upper_v, IntegerAttr[IndexType])
                    assert isa(step_v, IntegerAttr[IndexType])
                    lower_v, upper_v, step_v = (c.value.data for c in (lower_v, upper_v, step_v))
        ...
        for i, arg in reversed(list(enumerate(inner_loop.body.block.args))):
            if i in tile_sizes:
                ...
                inner_loop.body.block.insert_op_before(iv, inner_loop.body.block.first_op)
        ...
        rewriter.replace_matched_op([zero, *tile_sizes.values(), *outter_step, outter_loop])
...
class ScfParallelLoopTilingPass(ModulePass):
    ...
    def apply(self, ctx: Context, op: ModuleOp) -> None:
        walker = PatternRewriteWalker(
            GreedyRewritePatternApplier([ScfParallelLoopTilingPattern(tuple(self.parallel_loop_tile_sizes))]),
