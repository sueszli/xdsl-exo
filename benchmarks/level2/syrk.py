...
import exoblas.exo_blas_config as C
from exo import *
...
def schedule_macro(mk, precision, machine, max_N, max_K, m_r, n_r_fac, Trans):
    ...
    mk, cursors = pack_mem(
        mk,
        i_loop,
        mk.args()[3 + int("gemm" in mk.name())].name(),
        packed_A_shape,
        "packed_A",
        rc=1,
    )
    ...
    packed_A_alias_shape = (
        (isTrans, ceil(max_N / n_r)),
        (1 - isTrans, max_K),
        (isTrans, n_r),
    )
    mk, cursors = pack_mem(
        mk,
        i_loop,
        mk.args()[4 + int("gemm" in mk.name())].name(),
        packed_A_alias_shape,
        "packed_A_",
        rc=1,
    )
