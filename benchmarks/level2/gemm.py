...
def schedule_macro(gemm_mk, precision, machine, max_M, max_N, max_K, m_r, n_r_fac, TransA, TransB):
    ...
    packed_A_shape = (
        (int(isTransA), ceil(max_M / m_r)),
        (1 - int(isTransA), max_K),
        (int(isTransA), m_r),
    )
    ...
    gemm_mk, _ = extract_subproc(
        gemm_mk,
        gemm_mk.find_loop("i0o" if not isTransA else "i0").expand(0, 1 - int(isTransA)),
        gemm_mk.name() + "_A_pack",
    )
    isTransB = TransB == CblasTransValue
    packed_B_shape = (
        (1 - int(isTransB), ceil(max_N / n_r)),
        (int(isTransB), max_K),
        (1 - int(isTransB), n_r),
    )
    ...
    gemm_mk, _ = extract_subproc(
        gemm_mk,
        gemm_mk.forward(block).expand(0, 2 - 2 * int(isTransB)),
        gemm_mk.name() + "_B_pack",
    )
    gemm_mk = extract_and_schedule(schedule_compute)(gemm_mk, i_loop, gemm_mk.name() + "_compute", precision, machine, m_r, n_r_fac)
...
def schedule_gemm(gemm, i_loop, precision, machine, TransA=None, TransB=None):
    ...
    starter_gemm = gemm
    gemm = specialize(gemm, i_loop, "N <= 100")
