...
class IRGenerator:
    ...
    def _stmt_assign(self, stmt: LoopIR.Assign | LoopIR.Reduce) -> None:
        ...
        buffer = self.symbol_table[stmt.name]
    ...
    def _arg_type(self, arg: LoopIR.fnarg, body: list[LoopIR.stmt]) -> Attribute:
        ...
        if arg.type.is_tensor_or_window() or any(sym == arg.name for sym, _ in get_writes_of_stmts(body)):
    ...
    def _generate_procedure(self, procedure: LoopIR.proc) -> None:
        ...
        for arg, val in zip(procedure.args, block.args):
            self.symbol_table[arg.name] = self._buffer(val, arg.type) if isinstance(val.type, LLVMPointerType) else val
