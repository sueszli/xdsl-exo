...
# arith.cmpi predicate int -> (operator string, is_signed)
_CMPI_PRED: dict[int, tuple[str, bool]] = {
    0: ("==", True),
    1: ("!=", True),
    2: ("<", True),
    3: ("<=", True),
...
# arith.cmpf predicate int -> (operator string, is_ordered)
_CMPF_PRED: dict[int, tuple[str, bool]] = {
    1: ("==", True),
    2: (">", True),
    3: (">=", True),
    4: ("<", True),
...
# integer and float binary ops: op type -> llvmlite builder method name
...
def _emit_op(op: Operation, builder: ir.IRBuilder, block_map: BlockMap, phi_map: PhiMap, val_map: ValMap) -> None:
    # lowering arith could be done by default?
    if method := _BIN_OPS.get(type(op)):
    ...
    match op:
        ...
        case arith.NegfOp():
            val_map[op.result] = builder.fneg(val_map[op.operands[0]])
        case arith.CmpiOp():
            pred, is_signed = _CMPI_PRED[op.predicate.value.data]
            cmp_fn = builder.icmp_signed if is_signed else builder.icmp_unsigned
            val_map[op.result] = cmp_fn(pred, val_map[op.lhs], val_map[op.rhs])
        case arith.CmpfOp():
            pred, is_ordered = _CMPF_PRED[op.predicate.value.data]
            cmp_fn = builder.fcmp_ordered if is_ordered else builder.fcmp_unordered
            val_map[op.result] = cmp_fn(pred, val_map[op.lhs], val_map[op.rhs])
        case arith.SelectOp():
            val_map[op.result] = builder.select(val_map[op.cond], val_map[op.lhs], val_map[op.rhs])
